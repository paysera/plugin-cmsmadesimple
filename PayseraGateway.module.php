...
class PayseraGateway extends CGPaymentGatewayBase {
  ...
  function SetParameters()
  {
    ...
    $this->RegisterRoute('/[Ww]eb[Tt]o[Pp]ay\/ipn\/(?P<returnid>[0-9]+)$/', array('action'=>'ipn'));
  ...
  function SaveState($encrypt = false,$key = '')
  {
	...
	$_SESSION['webtopaygw'] = base64_encode($enc);
	return $key;
      }
    else
      {
	$raw = serialize($this->_data);
	$_SESSION['webtopaygw'] = base64_encode($raw);
	return '';
      }
  }
  ...
  {
    if( !isset($_SESSION['webtopaygw']) ) return FALSE;
    if( empty($key) )
      {
	$raw = base64_decode($_SESSION['webtopaygw']);
      }
    else
      {
	$enc = base64_decode($_SESSION['webtopaygw']);
	$raw = $this->decrypt($key,$enc);
	if( $raw === FALSE ) return FALSE;
      }
    $this->_data = unserialize($raw);
    unset($_SESSION['webtopaygw']);
    return TRUE;
  }
  ...
  /*---------------------------------------------------------
   ...
   ---------------------------------------------------------*/
	function GetForm($returnid){
		...
		$smarty->assign('image_url', $config['root_url'].'/modules/'.$this->GetName().'/images/webtopay_button.gif');
